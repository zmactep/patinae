// Ellipsoid picking — same impostor + ray-ellipsoid intersection as the
// colour shader; writes `PackedId(rep_kind=Ellipsoid, object_id, atom_id=
// group_id)` into Rg32Uint.
...
fn fs_main(input: VsOut) -> PickOut {
    ...
    // Override rasterized depth (bounding-quad z) with the analytical hit.
