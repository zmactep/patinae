// Line instance builder. One thread per object-local bond. Visibility
// gate: at least one endpoint must have the `LINES` rep enabled. Emits
// 1, 2, or 3 `LineInstance`s per visible bond based on bond order,
// using the CPU-precomputed local-plane perpendicular from BondGpu.
//
...
const VALENCE_JOIN_FACTOR: f32 = 0.5;
...
@group(1) @binding(0) var<uniform>             params:    LineBuildParams;
@group(1) @binding(1) var<storage, read_write> instances: array<LineInstance>;
...
fn cs_main(
    ...
    let vis_a = (atom_a.repr_flags & REP_BIT_LINES) != 0u;
    let vis_b = (atom_b.repr_flags & REP_BIT_LINES) != 0u;
    ...
    var bond_dir = vec3<f32>(0.0);
    if n > 1u {
        let axis = pb - pa;
        let axis_len = length(axis);
        if axis_len < 1e-6 { return; }
        bond_dir = axis / axis_len;
        perp = bond_valence_perp(bond, bond_dir);
    }

    for (var slot_id: u32 = 0u; slot_id < n; slot_id = slot_id + 1u) {
        let factor = offset_factor(order, slot_id);
        let off = perp * (factor * params.valence_scale);
        let axis_join = bond_dir * (abs(factor * params.valence_scale) * VALENCE_JOIN_FACTOR);
        let dst = atomicAdd(&raw_count[0], 1u);
        if dst >= arrayLength(&instances) {
            continue;
        }
        var ins: LineInstance;
        ins.p0_pad = vec4<f32>(pa + off - axis_join, 0.0);
        ins.p1_pad = vec4<f32>(pb + off + axis_join, 0.0);
