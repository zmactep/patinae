...
pub(crate) mod tests {
    #[test]
    fn core_crates_do_not_depend_on_concrete_plugins_or_language_bindings() {
        ...
        for package in metadata["packages"].as_array().unwrap() {
            ...
            for dependency in package["dependencies"].as_array().unwrap() {
                ...
                assert!(
                    !matches!(name, "pyo3" | "numpy"),
                    "{} depends on binding runtime {name}",
                    manifest.display()
                );
    ...
    use patinae_cmd::{
        parse_command, CommandContext, CommandExecutor, CommandRegistry,
        CommandRuntimeRequirements, OutputMessage,
    };
    ...
    pub(crate) struct SharedFixture {
        ...
        pub(crate) setting_names: Vec<&'static str>,
    }
    ...
    impl SharedFixture {
        ...
        pub(crate) fn shared(&self) -> SharedContext<'_> {
            SharedContext {
                tasks: None,
    ...
    fn command_input_with_full_session_preserves_registry() {
        ...
        let input =
            command_input_for_session(&mut session, CommandRuntimeRequirements::FULL_SESSION);
        let decoded = decoded_wire_session(&input.session);
    ...
    pub(crate) fn test_declaration(register: Option<PluginRegisterFn>) -> PluginDeclaration {
        PluginDeclaration {
    ...
    fn fixture_format_reader(
        _reader: Box<dyn std::io::Read>,
    ) -> Result<Vec<ObjectMolecule>, String> {
        Ok(Vec::new())
    }
    ...
    unsafe extern "C" fn fixture_panel_snapshot(
        ...
        let Ok(input): Result<patinae_plugin::wire::WireSharedInput, _> = fixture_decode(input)
        else {
            return AbiStatus::INVALID;
        };
        if input.wire_version != RUNTIME_WIRE_VERSION {
    ...
    unsafe extern "C" fn fixture_panel_event(
        ...
        let Ok(_input): Result<patinae_plugin::wire::WirePanelEventInput, _> =
            fixture_decode(input)
        else {
            return AbiStatus::INVALID;
        };
    ...
    unsafe extern "C" fn register_fixture(
        ...
        let command = AbiCommandDescriptor {
            ...
            argument_syntax: patinae_cmd::ArgumentSyntax::Pml as u8,
